  ...
  Account
    ...
    Public Instance Methods
      #transfer(amount, currency)
         scenario 'amount and currency has valid values'
           given amount 50
           given currency 'USD'
...
With an input like this
  ...
  describe Account do
    static_methods do
      new do
        describe_returns("Account with 0 USD") do
      ...
      new(arg(:amount), arg(:currency)) do
        scenario 'account and currency has valid values' do
          ...
          describe_return "an Account with given amount and currency" do
            it { balance.should == given.amount }
            it { currency.should == given.currency }
          end
        end

        scenario 'an unknown currency' do
          given do
            amount   = 50
            currency = 'UX!'
          end
  ...
  describe TransferDSL do
    static_methods do
      new(arg(:source_account) do
        given do
          source_account = Account.new(50, 'USD')
        end
        describe_return("A Transfer DSL with the given source account") do
    ...
    instance_methods do
      to(arg(:target_account, arg(:amount), arg(:currency)) do
        scenario 'source account has enough money' do
          given do
            target_account = Account.new(50, 'USD')
            amount = 5
            currency = 'USD'
            subject = TransferDSL.new(Account.new(50, 'USD')
          end
          describe_subject(:source_account, "is an account with 45 USD") do
             it { amount.should == 45 }
             it { currency.should == 'USD' }
          end
          describe_arg(:target_account,  "is an account with 5 USD") do
             it { amount.should == 5 }
             it { currency.should == 'USD' }
          end
        end
...
rspec-apigen also has support for fixture
  describe Person
    ...
    instance_methods do
      ...
      add_friend_relationship(arg(:to_person)) do
        given do
          to_person = Person.fixture(:me)
        end
        ...
