...
With an input like this
  ...
  #        it_behaves_like "Account with 0 USD" can also be used
      ...
      new(arg(:amount, 50), arg(:currency, 'USD')) do
        scenario 'account and currency has valid values'
        describe_return("Account with 50 USD") do
  ...
  describe TransferDSL do
    static_methods do
      new(arg(:source_account, :account_50)) do
        describe_return("A Transfer DSL with a source account") do
    ...
    instance_methods do
      to(arg(:target_account, Account.new), arg(:amount, 5), arg(:currency, 'USD')) do
        given(TransferDSL.new(Account.new(50, 'USD'), 5, 'USD')) do
          scenario 'source account has enough money'
          describe_arg(:target_account) do
             it "has amount 5" { amount.should == 5 }
             it "has currency 'USD" { currency.should == 'USD' }
          end
          describe_method(:source_account) do  # not sure about describe_method name ?
             it "has amount 45" { amount.should == 45 }
             it "has currency 'USD" { currency.should == 'USD' }
          end
        end
      end
